"""
Optimized Polynomial Convolution (PolyConv) method with Numba JIT
compilation and SQLite checkpointing.

Specialized for d-dimensional isotropic harmonic oscillator spectra.
For arbitrary potentials, use polyconv_method.py instead.
"""
...
def level_degeneracy_numba(d, k):
    """Degeneracy of shell k in d dimensions (numba-compatible)."""
    if k == 0:
        return 1.0
    if d == 1:
        return 1.0
...
@numba.njit
def log_binom(n, k):
    """Log of binomial coefficient C(n, k) (numba-compatible)."""
    if k < 0 or k > n:
        return -np.inf
...
def _polyconv_fixed_chunk(k_start, k_end, logZ, d, logb, n):
    """Process a chunk of shells with fixed iteration count."""
...
def fermion_logZ_numeric(tau, N, n, d, w=1.0, max_shell=None, tol=1e-4,
    ...
    tau : float
        Imaginary time.
    N : int
        Number of Trotter beads (must be positive).
    n : int
        Number of fermions.
    d : int
        Spatial dimension.
    ...
    max_shell : int or None
        Fixed maximum shell. If None, uses adaptive truncation.
    tol : float
        Convergence tolerance.
    consecutive_small : int
        Consecutive small changes for convergence.
    safety_cap : int
        Maximum shell index.
    return_all : bool
    ...
    init_polyconv_db(db_name)
    state = load_polyconv_state(n, d, b, db_name)
