...
function Z_and_derivative_recursive!(n::Int, d::Int, b::BigFloat, cache::Dict)
    ...
    for i in 1:n
        ...
        # --- Sum the terms directly --- BigFloat(10^(n-i))
        # For Z_n
...
function run_and_plot()
    ...
    for N in bead_counts
        ...
        @threads for i in eachindex(tau_values)
            ...
            # --- NEW: Call the direct-space recursive function ---
            Z_val, Z_prime = Z_and_derivative_recursive!(
                num_fermions, dimensions, b, cache2
            )
            ...
            energystar = if isnan(Z_prime) || Z_val <= 0 # Avoid division by zero or log of negative
                NaN
            else
                log_Z_prime = Z_prime / Z_val # This is the log derivative
                # The rest of the formula is the same
            ...
            if mode == "thermo"
                energy = (1.0 / sqrt(1 + (epsilon^2) /4))*energy1 + (w / sqrt(1 + (w^2) * (epsilon^2) / 4))*(energystar - energy1star)
            else
                energy = 0.5*(sqrt(1+(epsilon^2)/4) + 1.0 / sqrt(1 + (epsilon^2) /4))*energy1 + (w*0.5)* (sqrt(1 + (epsilon^2)*(w^2)/4)+ 1.0 / sqrt(1 + (w^2) * (epsilon^2) / 4))*(energystar - energy1star)
            end
