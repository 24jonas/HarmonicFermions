...
function run_and_plot()
    ...
    # Initialize DataFrames with both tau and N
    ...
    # Pre-allocate arrays for the single run
    energies_T = Vector{Float64}(undef, length(tau_values))
    energies_H = Vector{Float64}(undef, length(tau_values)) 

    print("... calculating energies for coupled tau and N values \n")
        
    # SINGLE THREADED EXECUTION
    for i in eachindex(tau_values)
        tau = tau_values[i]
        N = tau_beads[i] # Retrieve the specific bead count for this time step
    ...
    # Assign calculated energies to the DataFrames
    results_df_T[!, "Energy"] = energies_T
    results_df_H[!, "Energy"] = energies_H
