    ...
    for N in bead_counts
        ...
        @threads for i in eachindex(tau_values)
            ...
            Z_val_f, Z_prime_f = Z_and_derivative_recursive!(
                floor(Int,num_fermions/2), dimensions, b, cache
            )

            Z_val_c, Z_prime_c = Z_and_derivative_recursive!(
                ceil(Int,num_fermions/2), dimensions, b, cache2
            )

            # --- NEW: Update the energy calculation ---
            # The logarithmic derivative is Z'/Z.
            energy_f = if isnan(Z_prime_f) || Z_val_f <= 0 # Avoid division by zero or log of negative
                NaN
            else
                log_Z_prime = Z_prime_f / Z_val_f # This is the log derivative
                # The rest of the formula is the same
                if mode == "thermo"
                    Float64((lambda_val / gamma_val) * (b * log_Z_prime))
                else
                    Float64((gamma_val^2 + 1 )/(2*lambda_val)*(lambda_val / gamma_val) * (b * log_Z_prime))
                end
            end

            energy_c = if isnan(Z_prime_c) || Z_val_c <= 0 # Avoid division by zero or log of negative
                NaN
            else
