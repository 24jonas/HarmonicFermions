...
function run_and_plot()
    ...
    for N in bead_counts
        ...
        # Can be changed to parallel with @threads but it can create trouble with the compiler
        for i in eachindex(tau_values)
            ...
            factor_regular_T, factor_star_T =
                factor_calc_T(lambda_val, gamma_val, w, lambda_val_s, gamma_val_s)
            factor_regular_H, factor_star_H =
                factor_calc_H(lambda_val, gamma_val, w, lambda_val_s, gamma_val_s)

            energy_T =
                factor_regular_T * energy1 + factor_star_T * (energystar - energy1star)
            energy_H =
                factor_regular_H * energy1 + factor_star_H * (energystar - energy1star)
        ...
        # Save partial data
        CSV.write(joinpath(output_dir, "partial_data_$(run_id_T).csv"), results_df_T)
        CSV.write(joinpath(output_dir, "partial_data_$(run_id_H).csv"), results_df_H)
    end

    # --- Final Save ---
    output_filename_T = joinpath(output_dir, "data_$(run_id_T).csv")
    output_filename_H = joinpath(output_dir, "data_$(run_id_H).csv")
    CSV.write(output_filename_T, results_df_T)
    CSV.write(output_filename_H, results_df_H)
    print(results_df_H) # Helps to see quickly if more precision is needed (NaNs appearing)
    print("----\n")
end
