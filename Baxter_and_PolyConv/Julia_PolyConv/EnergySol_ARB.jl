...
function degeneracy(m::Int, d::Int)
    if d == 1 return 1 end
    if d == 2 return m + 1 end
    if d == 3 return div((m + 1) * (m + 2), 2) end
...
function harmEnergy(num_fermions::Int, dimensions::Int, b::ArbFloat) ################# NEW GRAND Z APPROACH
    ...
    while true
        ...
        C = [ArbFloat(0) for _ in 1:(max_k + 1)]
        C_prime = [ArbFloat(0) for _ in 1:(max_k + 1)]
        ...
        Z_new = [ArbFloat(0) for _ in 1:(num_fermions + 1)]
        Z_prime_new = [ArbFloat(0) for _ in 1:(num_fermions + 1)]
        
        for n in 0:num_fermions
            for k in 0:min(n, max_k)
...
function get_energy_calc(balanced::Bool)
    if balanced
        return (num_fermions, dimensions, b_s, ignored_cache) -> 
            harmEnergy(floor(Int, num_fermions / 2), dimensions, b_s) + harmEnergy(ceil(Int, num_fermions / 2), dimensions, b_s)
    else
        return (num_fermions, dimensions, b_s, ignored_cache) -> 
