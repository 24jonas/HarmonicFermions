...
function Get_heat_capacity()
    ...
    midpoint_taus = [(df_T.tau[i] + df_T.tau[i+1]) / 2 for i = 1:(n_tau-1)]
    ...
    for col in names(df_T)
        if col != "tau"
            results_df_T[!, col] =
                compute_heat_capacity(df_T.tau, df_T[!, col])/num_fermions
        end
    end
    ...
    for col in names(df_H)
        if col != "tau"
            results_df_H[!, col] =
                compute_heat_capacity(df_H.tau, df_H[!, col])/num_fermions
        end
    end
